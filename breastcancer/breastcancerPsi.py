...
for split, (trainval_index, test_index) in enumerate(skf.split(x.cpu().detach().numpy(),
    ...
    x_train, x_val, y_train, y_val = train_test_split(x_trainval, y_trainval, test_size=0.2, random_state=42) # end of adaptation
    ...
    layers = [
        torch.nn.Linear(x_train.shape[1], 45),
        torch.nn.Sigmoid(),
        torch.nn.Linear(45, 45),
        torch.nn.Sigmoid(),
        torch.nn.Linear(45, 1),
        torch.nn.Sigmoid(),
    ]
    ...
    for epoch in range(10001):
        ...
        optimizer.step()
        model = te.nn.functional.prune_equal_fanin(model, epoch, prune_epoch=10000) # Pruning
    ...
    # extracting FOL explanations for target class "malign"
...
base_dir = f'./results-breastcancer/'
df_result.to_csv(os.path.join(base_dir, 'resultspsi1.csv'))
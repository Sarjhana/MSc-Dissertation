...
for split, (trainval_index, test_index) in enumerate(skf.split(x.cpu().detach().numpy(),
    ...
    model = Explainer(n_concepts=n_concepts, n_classes=n_classes, l1=1e-3, lr=0.01,
                      explainer_hidden=[10,10], temperature=0.60)
    ...
    dict = {'model_accuracy': [results['model_accuracy']],'model_f1': [f1], 'explanations0': [f[0]['explanation']], 'explanations0_f1': [f[0]['explanation_accuracy']], 'explanations0_fidelity': [f[0]['explanation_fidelity']], 'explanations0_complexity': [f[0]['explanation_complexity']],
